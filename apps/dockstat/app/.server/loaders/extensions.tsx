...
export async function ExtensionLoader({
	...
	const [treatyResPlugins, treatyResDockstatConfig] =
		await Promise.all([
			api.plugins.all.get(),
	...
	const repos =
		treatyResDockstatConfig.data?.config.registered_repos || []
	const allow_untrusted_repo =
		Boolean(
			treatyResDockstatConfig.data?.config.allow_untrusted_repo
		) === true
