import Elysia, { type ParseError } from "elysia"
...
const DockStatElysiaHandlers = new Elysia()
	.derive(
		...
		({ headers }) => {
			const dockStatRequestId =
				headers["x-dockstatapi-requestid"] || http.requestId.getRequestID(true)
			return {
				dockStatRequestId,
			}
		}
	...
	.onAfterResponse(
		...
		({ request, dockStatRequestId }) => {
			...
			ElysiaLogger.debug(`Responded to ${request.method} ${request.url}`, reqId)
		}
	)
