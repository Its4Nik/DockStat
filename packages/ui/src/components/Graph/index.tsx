...
interface ContainerNodeData extends DefaultRecord {
  label: string;
  status: string;
  image?: string;
  hostId?: number;
...
interface ClientNodeData extends DefaultRecord {
  label: string;
  status: string;
  ipAddress?: string;
  port?: number;
...
interface HostNodeData extends DefaultRecord {
  label: string;
  status: string;
  ipAddress?: string;
}
  ...
  ({ data, selected }: NodeProps<Node<ContainerNodeData>>) => {
    ...
    return (
      ...
      >
        ...
        {data.image && (
          <div
            className="text-xs text-orange-300/70 truncate mb-1"
            title={data.image}
          >
            {data.image}
          </div>
...
export const nodeTypes = {
  client: memo(({ data, selected }: NodeProps<Node<ClientNodeData>>) => {
    const statusColor =
      data.status === "online"
        ? "#10b981"
        : data.status === "offline"
  ...
  host: memo(({ data, selected }: NodeProps<Node<HostNodeData>>) => {
    const statusColor =
      data.status === "online"
        ? "#10b981"
        : data.status === "offline"
