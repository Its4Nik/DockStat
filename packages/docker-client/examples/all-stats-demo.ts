...
async function demonstrateAllStats() {
  ...
  try {
    ...
    if (!isHealthy) {
      console.log("❌ Cannot connect to Docker daemon. Make sure Docker is running.")
      return
    }
    ...
    console.log(`   📦 Container Stats: ${allStats.containerStats.length} containers`)
    console.log(`   🖥️ Host Metrics: ${allStats.hostMetrics.length} hosts`)
    console.log(`   🕐 Timestamp: ${new Date(allStats.timestamp).toLocaleString()}`)
    ...
    if (allStats.containerStats.length > 0) {
      ...
      if (allStats.containerStats.length > 5) {
        console.log(`   ... and ${allStats.containerStats.length - 5} more containers`)
      }
    } else {
    ...
    if (allStats.hostMetrics.length > 0) {
      ...
      allStats.hostMetrics.forEach((metrics, index) => {
        const totalMemoryGB = (metrics.totalMemory / 1024 / 1024 / 1024).toFixed(2)
    ...
    const runningContainers = allStats.containerStats.filter((c) => c.state === "running").length
    const totalHosts = allStats.hostMetrics.length
    const totalImages = allStats.hostMetrics.reduce((sum, h) => sum + h.images, 0)
    const totalMemoryGB =
      allStats.hostMetrics.reduce((sum, h) => sum + h.totalMemory, 0) / 1024 / 1024 / 1024
    const totalCPUs = allStats.hostMetrics.reduce((sum, h) => sum + h.totalCPU, 0)

    console.log("\n📋 Summary:")
    console.log(`   🔹 Total Hosts: ${totalHosts}`)
    console.log(`   🔹 Total Containers: ${runningContainers}/${totalContainers} running`)
...
export { demonstrateAllStats, demonstrateWebSocketAllStats, performanceComparison }
