...
class DB {
  ...
  constructor(path: string, options?: DBOptions, baseLogger?: Logger) {
    ...
    if (options?.autoBackup?.enabled) {
      const res = helperSetupAutoBackup(
        this.dbPath,
        this.db,
        this.backupLog,
        options.autoBackup,
  ...
  private shouldCheckExistingTable<_T>(
    tableName: string,
  ...
  createTable<_T extends Record<string, unknown>>(
    ...
    this.dbLog.info(
      `Creating Table '${tableName}' with ${Object.keys(columns).length} columns`,
    );
    ...
    const canMigrate = allowMigration(
      options || {},
      tableName,
      this.migrationLog,
    );
    ...
    const tableAlreadyExists =
      this.shouldCheckExistingTable(tableName, options) &&
      tableExists(this.db, tableName);
      ...
      if (canMigrate) {
        ...
        if (!currentSchema) {
          this.migrationLog.info(
            "Schema of table not found; new table => no migration needed",
          );
        } else {
  ...
  >(
    ...
    const mergedJson = [
      ...new Set([...autoDetectedJson, ...userJson]),
    ] as Array<keyof _T>;
    const mergedBoolean = [
      ...new Set([...autoDetectedBoolean, ...userBoolean]),
    ] as Array<keyof _T>;
