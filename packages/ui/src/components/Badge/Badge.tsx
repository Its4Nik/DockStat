...
// simple deterministic hash to generate color
function stringToColor(str: string): string {
  ...
  return `hsl(${color}, 40%, 40%)` // vibrant mid-tone color
}
...
export const Badge: React.FC<BadgeProps> = ({
  ...
  if (unique) {
    const text = typeof children === "string" ? children : JSON.stringify(children) || "badge"
    style = { backgroundColor: stringToColor(text), color: "#fff" }
  }
