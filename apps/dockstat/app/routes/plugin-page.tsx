 ...
 * Supports data loading via loaders and action execution via the API.
 * Route: /p/:pluginId/*
 */
...
export default function PluginPage() {
  ...
  /**
   * Execute a specific loader by ID using the ClientAPI
   */
  const executeLoader = useCallback(
    async (loaderId: string): Promise<LoaderResult | null> => {
      ...
      try {
        // Use ClientAPI to call the DockStat API server
        const response = await ClientAPI.plugins
          .frontend({ pluginId: String(data.route.pluginId) })
          .loaders({ loaderId })
          .execute.post({
            path: data.routePath,
            state,
        ...
        if (response.error || !response.data) {
          console.error(`Failed to execute loader ${loaderId}:`, response.error)
          return null
        }
  ...
  /**
   * Execute an API action using the ClientAPI
   */
  const executeApiAction = useCallback(
    async (action: ResolvedAction, payload?: unknown) => {
      ...
      try {
        ...
        if (response.error || !response.data) {
          console.error(`Failed to execute action ${action.id}:`, response.error)
