...
class DockerWebSocketServer {
  ...
  private handleWebSocketOpen(ws: ServerWebSocket<{ connectionId: string }>): void {
    ...
    if (streamManager) {
      ...
      streamManager.on("message:send", (connId: string, message: DOCKER.StreamMessage) => {
  ...
  private handleWebSocketClose(ws: ServerWebSocket<{ connectionId: string }>): void {
    const { connectionId } = ws.data
    this.handleDisconnection(connectionId)
  }

  private handleWebSocketDrain(ws: ServerWebSocket<{ connectionId: string }>): void {
  ...
  private setupDockerEventListeners(): void {
    ...
    this.dockerClient.events.on("container:started", (hostId, containerId, containerInfo) => {
    ...
    this.dockerClient.events.on("container:stopped", (hostId, containerId, containerInfo) => {
    ...
    this.dockerClient.events.on("container:created", (hostId, containerId, containerInfo) => {
  ...
  private sendWelcomeMessage(connectionId: string): void {
    ...
    const welcomeMessage: {
      ...
      data: {
        message: string
        connectionId: string
        availableChannels: DOCKER.StreamChannel[]
        serverTime: string
      }
    } = {
      id: "welcome",
  ...
  public async getServerStats(): Promise<{
    ...
    const totalContainers = hostMetrics.reduce((sum, host) => sum + host.containers, 0)
    const runningContainers = hostMetrics.reduce((sum, host) => sum + host.containersRunning, 0)
