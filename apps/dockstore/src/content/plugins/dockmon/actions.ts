...
const DockMonActions = createActions<DockMonTable>({
	getAllMetrics: ({ table }) => table?.select(["*"]).all() ?? [],

	// Get container metrics only
	getContainerMetrics: ({ table }) =>
		table?.select(["*"]).where({ type: "CONTAINER" }).all() ?? [],

	// Get host metrics only
	getHostMetrics: ({ table }) =>
		table?.select(["*"]).where({ type: "HOST" }).all() ?? [],
	...
	getMetricsSummary: ({ table }) => {
		...
		const uniqueContainers = new Set(
			containerMetrics.map((m) => m.container_id).filter(Boolean),
		);
	...
	getDashboardData: ({ table }) => {
		...
		const totalImages = latestHosts.reduce(
			(sum, h) => sum + (h.data.host_metrics?.images ?? 0),
			0,
		);
