...
export class DockerClientManagerCore {
	...
	constructor(
		...
		for (const c of clients) {
			if (!c.id) {
				this.logger.warn(
					`Skipping client without id in DB row: ${JSON.stringify(c)}`
				)
				continue
			}
	...
	public async registerClient(
		...
		try {
			...
			const { id: clientId } =
				this.table.select(["id"]).where({ name }).first() ?? {}
			...
			const extra = dbStepDone
				...
				: ""
			const msg = `Error while registering Client ${name}${extra} - error: ${JSON.stringify(
				error
			)}`
	...
	public attachEventListener(wrapper: WorkerWrapper): void {
		...
		const listener = (event: MessageEvent) => {
			...
			if (isProxyEventEnvelope(payload)) {
				try {
					message = workerUtils.buildMessage.buildMessageFromProxy(
						payload
					) as EventMessage
				} catch (err) {
					this.logger.debug(
	...
	public listenForEvents({
		...
		if (!eventType && !clientId) {
			for (const wrapper of this.workers.values()) {
				...
				this.logger.debug(
					`Setting up listening for events on worker ${id} (${clientName})`
				)
				if (!messageListener) {
					this.attachEventListener(wrapper)
				} else {
	...
	public async getAllHosts(): Promise<
		Array<{ name: string; id: number; clientId: number }>
	> {
