...
const PluginElysiaInstance = new Elysia({ prefix: "/plugins", detail: { tags: ["Plugins"] } })
  .get("/all", () => PluginTable.select(["*"]).all())
  .get('/status', () => PluginHandler.getStatus())
  .post("/install", ({ body }) => PluginHandler.savePlugin(body), { body: DBPluginShema })
  .post("/install/url", ({ body }) => PluginHandler.installFromManifestLink(body), { body: t.String() })
  .post("/activate", async ({ body }) => await PluginHandler.loadPlugins(body),
    {
  ...
  .post("/delete", ({ body }) => PluginHandler.deletePlugin(body), { body: t.Number() })
  .post("/:id/routes/*", async ({ request, params }) => PluginHandler.handleRoute(request, params), { detail: { description: "This route proxies all Plugin-API requests to the specified Plugin's Elysia Instance" } });
