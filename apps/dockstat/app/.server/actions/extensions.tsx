...
import type { Route } from "../../routes/+types/dockstore"
import { api } from "../treaty"
import { logger } from "."

export async function ExtensionActions({ request }: Route.ActionArgs) {
  logger.debug("Parsing form Data")
  const formData = await request.formData()
  logger.debug("Parsing Plugin Manifest String to Object")
  const pluginManifest: DBPluginShemaT = JSON.parse(formData.get("pluginManifest")?.toString() || "{}")
