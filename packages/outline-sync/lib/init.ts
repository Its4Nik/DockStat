 ...
 */
export async function listCollectionsPrompt(opts: { dryRun?: boolean; nonInteractive?: boolean }) {
  ...
  const selection = await question("Select a collection by number (or press Enter to cancel): ")
  ...
  if (!exists) {
    ...
    logger.info(`Added collection to ${path.join("configs", "outline-sync.json")}`)
  } else {
 ...
 */
export async function bootstrapCollection(opts: { collectionId: string; dryRun?: boolean }) {
  ...
  logger.debug(`First 3 documents: ${JSON.stringify(docs.slice(0, 3), null, 2)}`)
  ...
  for (const n of map.values()) {
    ...
    if (!dryRun) {
      ...
      logger.debug(`[dry-run] would write ${filePath} (${content.length} bytes)`)
    }
  }
  ...
  if (!dryRun) {
    ...
    logger.debug(`[dry-run] would save pages to ${pagesFile} and config to ${configFile}`)
  }
