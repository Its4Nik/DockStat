...
export type EdenBody<T extends EdenRoute> =
...
export type EdenParams<TRoute extends EdenRoute> =
  Parameters<TRoute> extends [] ? undefined : Parameters<TRoute>[0];

export type SmartMutationInput<TRoute extends EdenRoute> =
  EdenParams<TRoute> extends undefined
...
export type ResponseData<TRoute extends EdenRoute> = NonNullable<
  EdenData<TRoute>
> & {
  message: string;
};
...
export type EdenQueryData<T extends EdenQueryRoute> = Awaited<
  ReturnType<T>
>["data"];
