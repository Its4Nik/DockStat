...
export async function saveTopConfig(cfg: TopConfig) {
  ...
  await fs.writeFile(TOP_CONFIG_FILE, `${JSON.stringify(cfg, null, 2)}\n`, "utf8")
}
...
export async function getCollectionFilesBase(collectionId: string): Promise<{
  ...
  const collectionConfig = topConfig.collections.find((c) => c.id === collectionId)

  // If no specific collection config found, use default paths
  if (!collectionConfig) {
    logger.warn(`Collection config for ${collectionId} not found, returning default paths`)
    return {
      pagesFile: path.join(CONFIG_DIR, `${collectionId}.pages.json`),
      configFile: path.join(CONFIG_DIR, `${collectionId}.config.json`),
      saveDir: "docs",
  ...
  return {
    pagesFile: collectionConfig.pagesFile || path.join(CONFIG_DIR, `${collectionId}.pages.json`),
    configFile: collectionConfig.configFile || path.join(CONFIG_DIR, `${collectionId}.config.json`),
...
export async function saveCollectionConfig(c: CollectionConfig) {
  ...
  const { configFile, configDir } = await getCollectionFilesBase(c.collectionId)
