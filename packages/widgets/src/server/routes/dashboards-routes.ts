  ...
  return new Elysia({
    ...
    .get(
      ...
      ({ params, status }) => {
        ...
        if (!dashboard) {
          return status(404, { error: `Dashboard "${params.id}" not found`, success: false as const })
        }
        return dashboard
      },
      {
    ...
    .post(
      ...
      },
      {
        body: t.Object({
          ...
          partial: t.Optional(t.Boolean({ description: "Allow import even with missing widgets" })),
        }),
        detail: {
          description: "Import a dashboard from a JSON manifest string. Validates that all required widgets exist.",
    ...
    .post(
      ...
      },
      {
        ...
        detail: {
          description: "Import a dashboard from a zip archive containing a manifest.json. Validates that all required widgets exist.",
