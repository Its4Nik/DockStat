	...
	implements DOCKER.DockerEventEmitterInterface
{
	...
	constructor(loggerParents: string[]) {
		super()
		this.logger = new Logger('DEM', loggerParents)
	...
	public emitContainerDied(hostId: number, containerId: string): void {
		...
		this.emit(
			'warning',
			`Container died: ${containerId.substring(0, 12)}`,
			{
				hostId,
				containerId,
			}
