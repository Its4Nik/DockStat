...
async function runComprehensiveTests() {
  ...
  await test.test("Initialize with default theme", () => {
    ...
    test.assertEqual(currentTheme.name, "default", "Default theme name should be 'default'")

    const themeName = db.getCurrentThemeName()
    test.assertEqual(themeName, "default", "Current theme name should be 'default'")
    db.close()
  })
  ...
  await test.test("Expose underlying DB instance", () => {
    ...
    test.assert(typeof rawDB.table === "function", "Should be a valid DB instance")
    db.close()
  })
  ...
  await test.test("Add custom theme", () => {
    ...
    test.assert(result !== null && result !== undefined, "Should successfully add theme")
    db.close()
  })
  ...
  await test.test("Get theme by name", () => {
    ...
    test.assert(retrievedTheme !== null && retrievedTheme !== undefined, "Should retrieve theme")
    if (retrievedTheme) {
      test.assertEqual(retrievedTheme.name, "custom-theme", "Theme name should match")
      test.assertEqual(retrievedTheme.version, "2.0.0", "Theme version should match")
      test.assertEqual(retrievedTheme.creator, "TestCreator", "Theme creator should match")
    }
    db.close()
  })
  ...
  await test.test("Get non-existent theme", () => {
    ...
    test.assertEqual(retrievedTheme, null, "Should return null for non-existent theme")
    db.close()
  })
  ...
  await test.test("Get all themes", () => {
    ...
    test.assert(themeNames.includes("custom-theme"), "Should include custom-theme")
    test.assert(themeNames.includes("second-theme"), "Should include second-theme")
  ...
  await test.test("Set current theme", () => {
    ...
    test.assert(result !== null && result !== undefined, "Should successfully set theme")

    const currentTheme = db.getCurrentTheme()
    test.assertEqual(currentTheme.name, "custom-theme", "Current theme should be updated")

    const currentThemeName = db.getCurrentThemeName()
    test.assertEqual(currentThemeName, "custom-theme", "Current theme name should be updated")
  ...
  await test.test("Update existing theme (OR REPLACE)", () => {
    ...
    if (retrievedTheme) {
      test.assertEqual(retrievedTheme.version, "3.0.0", "Theme version should be updated")
      test.assertEqual(retrievedTheme.creator, "UpdatedCreator", "Theme creator should be updated")
    }
  ...
  await test.test("Docker client integration pattern", () => {
    ...
    test.assert(typeof rawDB.table === "function", "Raw DB should have table method")
    ...
    test.assert(typeof rawDB.createTable === "function", "Raw DB should allow table creation")
  ...
  await test.test("Persistent theme configuration", () => {
    ...
    const currentTheme = db.getCurrentTheme()
    test.assertEqual(currentTheme.name, "custom-theme", "Theme should persist across instances")

    const currentThemeName = db.getCurrentThemeName()
    test.assertEqual(currentThemeName, "custom-theme", "Theme name should persist across instances")
  ...
  await test.test("Theme data serialization integrity", () => {
    ...
    if (retrieved) {
      ...
      test.assert("Aurora" in retrieved.vars.background_effect, "Should preserve Aurora effect")
  ...
  await test.test("Database utility methods", () => {
    ...
    test.assertEqual(dbPath, "dockstat.sqlite", "Should return correct database path")
