...
export async function getDefaultAuthKey(): Promise<string> {
  ...
  // Priority List
  let AuthPriority: DockStackAuthVariations[] = [
    "DevEnv",
    "ProdEnv",
    "InstanceUUID",
  ];
  ...
  if (DockStackAuthPriority && isDockStackPriorityList(DockStackAuthPriority)) {
    dockNodeAuthHandlerLogger.debug(
      `Found environment overwrite: ${DockStackAuthPriority}`
    );
    AuthPriority = DockStackAuthPriority.split(
      ","
    ) as DockStackAuthVariations[];
  }

  // Parsing Key Types
  dockNodeAuthHandlerLogger.info("Assembling DockStack Auth Variations");
  const KeyRecord: Record<DockStackAuthVariations, string> =
    {} as typeof KeyRecord;
  ...
  dockNodeAuthHandlerLogger.debug(
    `InstanceUUIDResponse: ${JSON.stringify(InstanceUUID)}`
  );
  ...
  dockNodeAuthHandlerLogger.error(
    `No Default Auth Key found in: ${JSON.stringify(KeyRecord)}`
  );
