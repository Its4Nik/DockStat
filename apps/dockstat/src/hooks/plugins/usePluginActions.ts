...
export function usePluginActions({
  ...
  const customHandlersRef = useRef<
    Map<string, (payload?: unknown) => void | Promise<void>>
  >(new Map());
  ...
  const handleAction = useCallback(
    async (actionId: string, payload?: unknown) => {
      ...
      const templateAction = parsedTemplate?.actions?.find(
        (a) => a.id === actionId,
      );
      ...
      if (actionDef.confirm) {
        const confirmed = window.confirm(
          actionDef.confirm.message || "Are you sure?",
        );
        if (!confirmed) return;
      }
      ...
      switch (actionDef.type) {
        ...
        case "reload":
          ...
          break;
        case "custom": {
          const handler = customHandlersRef.current.get(
            actionDef.handler || actionId,
          );
