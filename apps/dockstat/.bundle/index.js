  ...
  exports.install = (options) => {
    ...
    if (options.environment) {
      ...
      if (["node", "browser", "auto"].indexOf(environment) === -1) {
        throw new Error(
          "environment " + environment + " was unknown. Available options are {auto, browser, node}"
        )
      }
    }
  ...
  formatMessage(level, message, requestID) {
    ...
    const nameChain =
      this.parents.length > 0
        ? `${source_default.cyan(this.name)}${source_default.yellow(NAME_SEP + this.parents.join(NAME_SEP))}`
        : source_default.cyan(this.name)
    const reqFrom = requestID && this.reqFrom[requestID]
    const requestTag =
      requestID != null
        ? source_default.gray(
            `(${this.colorByReqID(requestID)}${reqFrom ? `@${source_default.green(reqFrom)}` : ""}) `
          )
        : ""
...
export { meta, dockmon_default as default }
