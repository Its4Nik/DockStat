...
export default class MonitoringManager {
	...
	private startHealthChecks(): void {
		logger.debug(
			`Starting health checks at an interval ${this.options.healthCheckInterval}ms`
		)
	...
	private async monitorContainerChanges(): Promise<void> {
		const changePromises = this.hosts.map(async (host) => {
			try {
				...
				this.detectContainerChanges(
					host.id,
					lastContainers,
					currentContainers
				)
				this.state.lastContainerStates.set(
					`host-${host.id}`,
					currentContainers
				)
			} catch (error) {
	...
	private detectContainerChanges(
		...
		const lastContainerMap = new Map(
			lastContainers.map((c) => [c.id, c])
		)
	...
	private async getContainerInfo(
		...
		const containerInfo = await this.withRetry<
			DOCKER.DockerAPIResponse['containerInspect']
		>(() => container.inspect())
	...
	private async getHostMetrics(
		...
		if (!docker || !host) {
			throw new Error(
				`Docker instance or host not found for ID ${hostId}`
			)
		}

		const [info, version] = await Promise.all([
			this.withRetry<DOCKER.DockerAPIResponse['systemInfo']>(() =>
				docker.info()
			),
