...
export function calculateNodeLayout(input: GraphInput): {
  ...
  const addNodeToGraph = (
    id: string,
    type: "Client" | "Host" | "Container" | "DockNode",
  ) => {
  ...
  for (const co of containers)
    addNodeToGraph(getContainerId(co.id), "Container");
  ...
  containers.forEach((container) => {
    ...
    if (g.hasNode(source)) {
      ...
      logger.warn(
        `Container ${container.id} references non-existent Host ${source}. Edge skipped.`,
      );
    }
  });
  ...
  logger.info(
    `Graph layout finished. Generated ${nodes.length} nodes and ${edges.length} edges.`,
  );
