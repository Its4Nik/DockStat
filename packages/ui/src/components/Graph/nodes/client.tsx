...
const statusStyleMap = {
  online:
    "text-success border-badge-success-outlined-border bg-badge-success-bg/10",
  offline:
    "text-badge-error-outlined-text border-badge-error-outlined-border bg-badge-error-bg/10",
};

export const ClientNode = memo(
  ({ data, selected }: NodeProps<Node<ClientNodeData>>) => {
    const badge =
      data.status === "online" ? statusStyleMap.online : statusStyleMap.offline;
        ...
        <div className="space-y-1 text-xs">
          {data.ipAddress && (
            <div className="flex justify-between gap-3">
              <span className="text-graph-client-text-secondary">
                Endpoint:
              </span>
