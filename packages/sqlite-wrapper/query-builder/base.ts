...
export abstract class BaseQueryBuilder<T extends Record<string, unknown>> {
	...
	protected deleteLogger = new Logger(
		"Delete",
		this.logger.getParentsForLoggerChaining()
	)
	protected insertLogger = new Logger(
		"Insert",
		this.logger.getParentsForLoggerChaining()
	)
	protected selectLogger = new Logger(
		"Select",
		this.logger.getParentsForLoggerChaining()
	)
	protected updateLogger = new Logger(
		"Update",
		this.logger.getParentsForLoggerChaining()
	)
	protected whereLogger = new Logger(
		"Where",
		this.logger.getParentsForLoggerChaining()
	)
	 ...
	 */
	protected getLogger(
		type: "DELETE" | "INSERT" | "SELECT" | "UPDATE" | "WHERE"
	) {
		switch (type) {
			case "DELETE":
	...
	protected transformRowFromDb(row: unknown): T {
		...
		try {
			...
			if (this.state.parser?.MODULE) {
				for (const [func, options] of Object.entries(
					this.state.parser.MODULE
				)) {
					const transpiler = new Bun.Transpiler(options)
					const funcKey = String(func)

					if (
						transformed[funcKey] !== undefined &&
						transformed[funcKey] !== null
					) {
						const compiled = transpiler.transformSync(
							String(transformed[funcKey])
						)
						...
						this.logger.debug(
							`Created Object URL for importing: ${transformed[funcKey]}`
						)
					}
				}
			}
