...
async function basicExample() {
  ...
  try {
    ...
    console.log(`Total Memory: ${(systemInfo.MemTotal / 1024 / 1024 / 1024).toFixed(2)} GB`)
    ...
    for (const container of containers.slice(0, 5)) {
      ...
      console.log(`    Created: ${new Date(container.created * 1000).toLocaleDateString()}`)
    }
    ...
    if (runningContainers.length > 0) {
      ...
      const containerStats = await dockerClient.getContainerStatsForHost(host.id)
    ...
    if (runningContainers.length > 0) {
      ...
      try {
        ...
        const detailedInfo = await dockerClient.getContainer(host.id, testContainer.id)
        ...
        const logs = await dockerClient.getContainerLogs(host.id, testContainer.id, {
          tail: 5,
          timestamps: true,
        })
