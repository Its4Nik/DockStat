 ...
 * ```
 */
export function isAuthenticatedUser(user: AuthUser | undefined): user is AuthUser {
 ...
 * ```
 */
export function requireAuth(context: { user?: AuthUser; isAuthenticated: boolean }): AuthUser {
 ...
 * ```
 */
export async function verifyWsToken(token: string | null): Promise<AuthUser | null> {
 ...
 * ```
 */
export function getWsUser(ws: { data: Partial<AuthWsContext> }): AuthUser | null {
...
/**
 * Type-safe WebSocket handler wrapper for authenticated WebSocket routes
 * Automatically verifies token and rejects unauthenticated connections
 *
 * Note: The token should be passed as a query parameter in the WebSocket URL
...
/**
 * Alternative WebSocket authentication helper for manual token handling
 * Use this when you need custom token extraction logic
 *
 * @example
 * ```typescript
 * app.ws("/ws", {
 *   open: async (ws) => {
 *     const token = extractTokenFromCustomSource(ws)
 *     if (await handleWsAuthentication(ws, token)) {
 *       // Authentication successful, user is now available in ws.data.user
 *       ws.send(`Welcome ${ws.data.user?.name}`)
 *     }
 *   }
 * })
 * ```
 ...
 * ```
 */
export async function handleWsAuthentication(
