...
class TestRunner {
      ...
      if (result instanceof Promise) {
        return result
          ...
          .catch((error) => {
            ...
            this.results.failed++
            this.results.errors.push(`${this.currentSection}: ${description} - ${error.message}`)
          })
      }
      console.log(`  ✅ ${description}`)
      this.results.passed++
  ...
  assertGreaterThan(actual: number, expected: number, message?: string) {
    if (actual <= expected) {
      throw new Error(`${message || "Not greater than"}: expected ${actual} > ${expected}`)
    }
  }

  assertArrayLength(array: unknown[], expectedLength: number, message?: string) {
...
async function runComprehensiveTests() {
  ...
  try {
    ...
    test.test("Single insert", () => {
      ...
      test.assertGreaterThan(result.insertId, 0, "Insert ID should be generated")
    ...
    test.test("Select specific columns", () => {
      const users = db.table<User>("users").select(["id", "name", "email"]).all()
      test.assertGreaterThan(users.length, 0, "Should return users")
    ...
    test.test("Select with WHERE conditions", () => {
      const adminUsers = db.table<User>("users").where({ type: "super_admin" }).all()
    ...
    test.test("Select with complex WHERE conditions", () => {
      const users = db
        ...
        .whereNotNull("email")
        .all()
      test.assertGreaterThan(users.length, 0, "Should find users matching complex conditions")
    })

    test.test("Select with raw WHERE expression", () => {
      const users = db.table<User>("users").whereRaw("name LIKE ?", ["%Test%"]).all()
      test.assertGreaterThan(users.length, 0, "Should find users with 'Test' in name")
    })

    test.test("Select with ordering", () => {
      const users = db.table<User>("users").orderBy("name").asc().limit(5).all()
      test.assertArrayLength(users, 5, "Should limit to 5 users")

      const descUsers = db.table<User>("users").orderBy("id").desc().limit(3).all()
      test.assertArrayLength(descUsers, 3, "Should limit to 3 users")
    })

    test.test("Select with offset and limit", () => {
      const users = db.table<User>("users").orderBy("id").offset(5).limit(10).all()
    ...
    test.test("Count records", () => {
      ...
      const adminCount = db.table<User>("users").where({ type: "super_admin" }).count()
      test.assertGreaterThan(adminCount, 0, "Should count admin users")
    })

    test.test("Check exists", () => {
      const exists = db.table<User>("users").where({ type: "super_admin" }).exists()
      test.assert(exists, "Should find existing admin user")

      const notExists = db.table<User>("users").where({ type: "nonexistent" }).exists()
      test.assert(!notExists, "Should not find nonexistent type")
    })

    test.test("Get single value", () => {
      const name = db.table<User>("users").where({ type: "super_admin" }).value("name")
      test.assert(name !== null, "Should return name value")
    })

    test.test("Pluck column values", () => {
      const names = db.table<User>("users").where({ type: "batch_user" }).limit(5).pluck("name")
    ...
    test.test("Simple update", () => {
      const result = db.table<User>("users").where({ type: "tester" }).update({ active: false })
    ...
    test.test("Update with regex conditions (known rowid limitation)", () => {
      ...
      test.assertGreaterThan(insertResult.changes, 0, "User should be inserted")
      ...
      test.assertGreaterThan(regexMatches.length, 0, "SELECT with regex should work")
    ...
    test.test("Increment operation", () => {
      ...
      const result = db.table("counters").where({ id: 1 }).increment("value", 5)
      test.assertEqual(result.changes, 1, "Should increment counter")

      const counter = db.table<{ id: number; value: number }>("counters").where({ id: 1 }).get()
      test.assertEqual(counter?.value, 15, "Value should be incremented to 15")
    })

    test.test("Decrement operation", () => {
      const result = db.table("counters").where({ id: 1 }).decrement("value", 3)
    ...
    test.test("Delete with WHERE conditions", () => {
      const result = db.table<User>("users").where({ active: 0 }).where({ type: "tester" }).delete()
      test.assertGreaterThan(result.changes, 0, "Should delete inactive tester users")
    })
    ...
    test.test("Delete and get", () => {
      ...
      const deletedUsers = db.table<User>("users").where({ type: "delete_test" }).deleteAndGet()
    ...
    test.test("Soft delete", () => {
      ...
      const result = db.table<User>("users").where({ type: "soft_test" }).softDelete()
      ...
      test.assert(user?.deleted_at !== null, "User should have deleted_at timestamp")
    })

    test.test("Restore soft deleted", () => {
      const result = db.table<User>("users").where({ type: "soft_test" }).restore()
      test.assertEqual(result.changes, 1, "Should restore soft deleted user")

      // Verify deleted_at is cleared
      const user = db.table<User>("users").where({ type: "soft_test" }).get()
      test.assert(user?.deleted_at === null, "User should have deleted_at cleared")
    })

    test.test("Delete older than timestamp", () => {
      const oldTimestamp = Math.floor(Date.now() / 1000) - 3600 // 1 hour ago
      const result = db.table<User>("users").deleteOlderThan("created_at", oldTimestamp)
      // This might not delete anything if all users are recent
      test.assert(result.changes >= 0, "Should execute delete older than query")
    })
    ...
    test.test("Delete duplicates", () => {
      ...
      test.assertGreaterThan(result.changes, 0, "Should delete duplicate users")
    })
    ...
    test.test("Direct database access", () => {
      ...
      const result = directDb.prepare("SELECT COUNT(*) as count FROM users").get() as {
        count: number
      }
    ...
    test.test("Debug regex filtering issue", () => {
      ...
      console.log(`  [DEBUG] Select with regex found: ${selectResult.length} rows`)
      ...
      console.log(`  [DEBUG] Manual regex test: ${manualMatch} (email: ${testEmail})`)
      ...
      console.log(`  [DEBUG] Update with regex changed: ${updateResult.changes} rows`)
    ...
    test.test("Complex query with all WHERE methods", () => {
      ...
      test.assert(users.length >= 0, "Complex query should execute without error")
    })

    test.test("Test with WITHOUT ROWID table", () => {
      db.createTable("keyvalue", { key: "TEXT PRIMARY KEY", value: "TEXT" }, { withoutRowId: true })

      const result = db.table<{ key: string; value: string }>("keyvalue").insert({
        key: "test_key",
        value: "test_value",
      })
      test.assertGreaterThan(result.changes, 0, "Should insert into WITHOUT ROWID table")
    })
  } catch (error) {
    test.addError(error)
  } finally {
